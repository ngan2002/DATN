...
namespace DATN.CheckIn_CheckOut
{
    public class CheckInPage
    {
        ...
        public IWebElement FindCheckInRowByInfo(string roomNumber, string customerName, DateTime checkIn)
        {
            ...
            for (int page = 0; page < maxPages; page++)
            {
                ...
                foreach (var row in rows)
                {
                    ...
                    if (cells.Count >= 5)
                    {
                        ...
                        string checkInText = cells[3].Text.Trim();  
        ...
        {
            return FindCheckInRowByInfo(roomNumber, customerName, checkIn) != null;
        }
        ////------------Đặt Phòng
        ...
        public IWebElement FindBookingRow(string roomNumber, string customerName, DateTime checkInDate, string expectedStatus = null)
        {
            wait.Until(driver => driver.FindElements(By.CssSelector("table tbody tr")).Count > 0);
                ...
                foreach (var row in rows)
                {
                    ...
                    if (cells.Count >= 6)
                    {
                        ...
                        bool match =
                            ...
                            checkInText == checkInDate.ToString("dd/MM/yyyy");
                        ...
                        if (match)
                        {
                            Console.WriteLine($"Tìm thấy dòng khớp: {extractedName} - {extractedRoom} - {checkInText} - {status}");
                            return row;
                        }
                    }
                }
